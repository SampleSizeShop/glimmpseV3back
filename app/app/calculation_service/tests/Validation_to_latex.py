...
def tex_table(file_path, output_name, V3_JSON: [], V2_results):
    ...
    _df_output = _df_output.round(
        {'Power': 7, 'SAS_Power': 7, 'deviation_sas_v3': 7, 'Sim_Power': 7, 'Power_v2': 7, 'deviation_sas_v3': 7, 'deviation_sim_v3': 7, 'deviation_v2_v3': 7})
    ...
    _df_print = _df_output[
        ['GLIMMPSE V3 Power', 'SAS Power (deviation)', 'Sim Power (deviation)', 'GLIMMPSE V2 Power (deviation)', 'Test', 'Sigma Scale', 'Beta Scale', 'Total N',
         'Alpha', 'Time']]

    _df_print = _df_print[_df_print['Test'].notna()]
    _df_output = _df_output[_df_output['Test_x'].notna()]

    timings = pd.DataFrame({'Timing for': ['GLIMMPSE V3', 'GLIMMPSE V2','Simulation'], 'Total': [_df_output["Time"].sum(), _df_v2summary['Name']['Total Calculation Time'], _df_v2summary['Name']['Total Simulation Time']], 'Mean': [_df_output["Time"].mean(), _df_v2summary['Name']['Mean Calculation Time'], _df_v2summary['Name']['Mean Simulation Time']]})
    deviations = pd.DataFrame({'GLIMMPSE V3 deviation from': ['Simulation', 'SAS', 'GLIMMPSE V2'], 'Max. Deviation': [ _df_output["deviation_sas_v3"].max(), _df_output["deviation_sim_v3"].max(), _df_output["deviation_v2_v3"].max()]})

    return timings.to_latex(index=False), deviations.to_latex(index=False), _df_print.to_latex(index=False, longtable=True), list_inputs
...
# test7_results = tex_table_test7(file_path, TEST_7_FILENAME, ['Example_7_Power_for_a_time_by_treatment_interaction_using_orthogonal_polynomial_contrast_for_time.json'], 'Example_7_Power_for_a_time_by_treatment_interaction_using_orthogonal_polynomial_contrast_for_time.csv')
...
all_names = [TEST_1_FILENAME, TEST_2_FILENAME, TEST_3_FILENAME]
all_titles = [TEST_1_TITLE, TEST_2_TITLE, TEST_3_TITLE]
all_descriptions = [TEST_1_STUDY_DESIGN_DESCRIPTION, TEST_2_STUDY_DESIGN_DESCRIPTION, TEST_3_STUDY_DESIGN_DESCRIPTION]
all_list_inputs = [test1_list_inputs, test2_list_inputs, test3_list_inputs]
all_timings = [test1_timings, test2_timings, test3_timings]
all_deviations = [test1_deviations, test2_deviations, test3_deviations]
all_results = [test1_results, test2_results, test3_results]
